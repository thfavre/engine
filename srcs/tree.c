...
int	on_update(t_engine *engine)
{
	...
	for (int x=100; x<1900; x+=90)
		for (int y=100; y<1040; y+=90)
			engine_draw_circle(&engine->img, (t_circle){(t_vector2){x, y}, 40, 12, COLOR_FIREBRICK4});

	// t_rect rect;
	// rect.size.x = 8;
	// rect.size.y = rect.size.x;
	// int mx, my;
	// mlx_mouse_get_pos(engine->win, &mx, &my);
	...
	// 		rect.pos.x = x;
	// 		rect.pos.y = y;
	// 		// engine_draw_rect(&engine->img, rect, engine_rgb(mx/2+i, my/2+j, (i*1.5 + j)/1000)); //0x119000 + i/2 + j*1.4 + COLOR_OFFSET
